...
def remove_from_order(parameters: dict, session_id: str):
    ...
    fulfillment_text += f' Your current order does not have {",".join(no_such_items)}' if no_such_items else ''
    if not current_order:
        fulfillment_text += " Your order is empty!"
    else:
        order_str = generic_helper.get_str_from_food_dict(current_order)
        fulfillment_text += f" Here is your updated order: {order_str}"
...
def track_order(parameters: dict, session_id: str):
    ...
    if order_status:
        fulfillment_text = f"Your order with order id {order_id} is currently {order_status}."
    else:
        fulfillment_text = f"Sorry, I couldn't find any order with order id {order_id}. Please check and provide the correct order id."
    return JSONResponse(content={"fulfillmentText": fulfillment_text})
